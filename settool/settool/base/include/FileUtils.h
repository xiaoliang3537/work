...
//                shortPath[iPathIndex] = '\\';
    ...
    *  文件拷贝
    */
    inline bool copyFile( char r_szSrcPath[MAX_PATH_LEN],char r_szDesPath[MAX_PATH_LEN],bool bForce)
    {
#ifdef _WIN32
        ...
        {
            std::cout<<"file not exist:"<<r_szSrcPath<<std::endl;
            return false;
        }
        FILE * pFile = fopen(r_szSrcPath,"rb");
        if(!pFile)
        {

            std::cout<<"copy file to read fopen failed:"<<r_szSrcPath<<std::endl;
            return false;
        }
        fseek(pFile,0,SEEK_END);
        unsigned int iLen = ftell(pFile);
        fseek(pFile,0,SEEK_SET);
        char *pData = new char[iLen];
        fread(pData,1,iLen,pFile);
        fclose(pFile);

        pFile = fopen(r_szDesPath,"wb");
        if(!pFile)
        {
            if(pData)
            {
            ...
            std::cout<<"copy file to write fopen failed:"<<r_szDesPath<<std::endl;
            return false;
        }
...
//        system(szBuf);
////        remove(r_szDesPath);
////        rename(r_szSrcPath,r_szDesPath);
        return true;
#endif
    ...
    *  文件拷贝目录
	*/
	inline bool copyFile( char r_szSrcPath[MAX_PATH_LEN],char r_szDesPath[MAX_PATH_LEN])
	{
#ifdef _WIN32
		...
		do
		{
			...
			if(FindFileData.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY)
			{
				if(strcmp(FindFileData.cFileName,"."))
				{
					if(strcmp(FindFileData.cFileName,".."))
					{
						...
						wsprintf(l_szNewDesPath,"%s%s\\",l_szDesPath,FindFileData.cFileName);

						wsprintf(l_szNewSrcPath,"%s%s\\",l_szSrcPath,FindFileData.cFileName);
			...
			else
			{
				...
				wsprintf(l_szDesFile,"%s%s",l_szDesPath,FindFileData.cFileName);
				wsprintf(l_szSrcFile,"%s%s",l_szSrcPath,FindFileData.cFileName);
...
//        }
	...
	*/
    ...
    inline static std::string& repleaceAllStr(std::string& text,char* str,char*strDest)
    {
        if(!text.empty())
        {
            while(true)
            {
                ...
                text.replace(iPos,strlen(str),strDest);
            }
        }
        return text;
    }
